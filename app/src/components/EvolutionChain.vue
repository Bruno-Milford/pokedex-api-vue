  ...
  <div>
    <v-row class="mx-0 d-flex align-center">
      <template v-for="(evolution_detail, index) in evolutions()">
        <v-col cols="12" md="3" :key="`evolution-${index}`" v-if="typeof evolution_detail == 'object'">
    ...
    evolutions() {
      ...
      while(evolution.species) {
        if(evolution.evolves_to.length > 0) {
          ...
          if(evolution.evolution_details.length > 0 && evolution.evolution_details[0].min_level) {
